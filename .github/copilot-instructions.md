...
- Run `python3 generate.py` from the repository root after changing `config/sofle.keymap` or the visualizer logic. The script reads `config/sofle.keymap` and rewrites `index.html`.
...
- `build.yaml` currently defines two `nice_nano_v2` builds for `sofle_left` and `sofle_right`, both with `nice_view_adapter nice_view_gem`; the left build also adds `-DCONFIG_ZMK_STUDIO=y`.
...
- `config/sofle.keymap` is the source of truth for keyboard behavior. It defines the Sofle CH-DE layout plus number, symbol, and conditional system layers.
...
- `config/sofle.conf` contains runtime feature flags for this keyboard, including BLE tuning and the nice!view Gem display settings.
...
- `generate.py` is a repository-specific tool, not a generic helper. It parses `config/sofle.keymap` directly with regexes, extracts layer and behavior metadata, and generates `index.html`, including the info box for conditional layers and mod-morph behaviors.
...
- Keep `config/sofle.keymap` in the current DTS-style structure. `generate.py` depends on recognizable `behaviors { ... }`, `conditional_layers { ... }`, and `keymap { ... }` blocks plus the `#define` layer indices near the top of the file.
