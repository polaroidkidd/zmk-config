...
- `build.yaml` currently defines `sofle_choc_pro_left` and `sofle_choc_pro_right` builds, both with the `sharp_mip` shield.
...
- `config/west.yml` pins external dependencies: ZMK itself comes from `zmkfirmware` at `v0.3`, while the Sofle Choc Pro board and `sharp_mip` display shield come from `Keebart/zmk-config`. `zephyr/module.yml` exposes this repository as a Zephyr module with `board_root: .`.
